...
def get_model(cfg):
    ...
    c4 = resblock_down(c3, cfg['D_CONV_CH']*4, neg_slope=cfg['RELU_NEG_SLOPE'], down_size=1)
    
    feature = layers.ReLU(negative_slope=cfg['RELU_NEG_SLOPE'])(c4)
    if cfg['D_USE_GSP']:
    ...
    thermal_rec = resblock_down(layers.Concatenate(axis=-1)([c3, c4]), cfg['D_CONV_THERMAL_CH'], down_size=1, neg_slope=cfg['RELU_NEG_SLOPE'])
