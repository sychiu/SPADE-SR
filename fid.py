...
def calc_fid(real_stats_path, gen_images, batch_size=64, rev_rgb=True):
    
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    real_stats = np.load(real_stats_path)
    gen_images = gen_images * 127.5 + 127.5
    if rev_rgb:
        gen_images = gen_images[:,:,:,::-1]
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        m1, s1 = real_stats['mu'][:], real_stats['sigma'][:]
        #print('calculate_activation_statistics')
        m2, s2 = calculate_activation_statistics(gen_images, sess, batch_size=batch_size)
...
#images = np.load('data_final/rgb.npy')
#images = shuffle(images)
#images = images[:10000]
#fid_value = calc_fid('fid_data/real_stats_full.npz', images, batch_size=256)
#print(fid_value)
...
if __name__ == "__main__":
    ...
    print("FID: ", fid_value)
    with open("score_ia85_wr_25R1_rec0_300", "a") as f:
        f.write("%.3f,\n" % (fid_value))
#python fid.py ~/fetch_bus/grideye_sasaya/fid_data/ia85_wr_25R1_rec0_300 rgb85_ia.npz --gpu 0
'''